...
with st.sidebar:
    ...
    st.markdown("[获取OpenAI的API秘钥](https://platform.openai.com/api-keys)")

subject = st.text_input("请输入视频的主题")
video_length = st.number_input("请输入视频的大致时长（单位：分钟）",min_value=0.1,step=0.1)
creativity = st.slider("请输入视频脚本的创造力，数字小代表严谨，数字大代表多样",min_value=0.0,max_value=1.0, value=0.3,step=0.1)

submit = st.button("创作脚本")
